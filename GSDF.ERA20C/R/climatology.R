# Functions to make (and access) climatologies from the ERA_20C data
...
ERA20C.make.climatology<-function(variable,month=NULL,first.year=1981,last.year=2010) {
  ...
  for(year in seq(first.year,last.year)) {
      ...
      mnth.file<-ERA20C.hourly.get.file.name(variable,year,mnth,15,0)
      ...
      if(nmd==29) nmd<-28 # death to leap years
       ...
       slab<-GSDF.ncdf.load(mnth.file,
                            ERA20C.translate.for.variable.names(variable),
     ...
     gc(verbose=FALSE )
      ...
      fn<-ERA20C.hourly.get.file.name(variable,year,mnth,15,0,type='normal')
      if(!file.exists(dirname(fn))) dir.create(dirname(fn),recursive=TRUE)

      GSDF.ncdf.write(result,fn,name=ERA20C.translate.for.variable.names(variable))
   }
}  
