# Scripts to retrieve CERA20C data from the ECMWF public data API.

# Pull down CERA20C data for a month from ECMWF (to $SCRATCH)
...
# Analyis variables
CERA20C.vars.a<-list(air.2m   = '167.128',
                     uwnd.10m = '165.128',
	             vwnd.10m = '166.128',
	             prmsl    = '151.128',
	             icec     = '31',
...
# Forecast variables
CERA20C.vars.f<-list(prate    = '228.128')
...
#' @param year
#' @param month
CERA20C.fetch.data.for.month<-function(var,year,month) {
    if(!is.null(CERA20C.vars.a[[var]])) {
        return(CERA20C.fetch.analysis.data.for.month(var,year,month))
    }
    if(!is.null(CERA20C.vars.f[[var]])) {
        return(CERA20C.fetch.forecast.data.for.month(var,year,month))
    }
...
#' @param year
#' @param month
CERA20C.fetch.analysis.data.for.month<-function(var,year,month) {

    if(is.null(CERA20C.vars.a[[var]])) {
        stop(sprintf("Unsupported analysis variable %s",var))
    }
    target.dir<-sprintf("%s/hourly/%04d/%02d",CERA20C.get.data.dir(),year,month)
...
#' @param year
#' @param month
CERA20C.fetch.forecast.data.for.month<-function(var,year,month) {
